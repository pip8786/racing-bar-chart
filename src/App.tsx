  ...
  return (
      <React.Fragment>
		...
		<div className="buttonContainer">
			...
			<button onClick={() => setIteration(iteration => Math.min(iteration + 1, data.length))} title="Next">
				<Icon type={Icons.chevronRight}/>
			</button>
			<button onClick={() => setIteration(iteration => Math.min(iteration + 10, data.length))} title="Skip 10">
				<Icon type={Icons.chevronsRight}/>
			</button>
			<button onClick={() => setIteration(0)} title="Reset">
				<Icon type={Icons.reset}/>
			</button>
		</div>

        <p>{data[iteration].season} - Game Day {data[iteration].gameDay} - {new Date(data[iteration].timestamp).toLocaleDateString()}</p>
      </React.Fragment>
  );
}
